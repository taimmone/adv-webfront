...
import { getServerValue } from '../mocks/server';
import { renderWithReduxAndMemoryRouter, renderWithReduxAndRouter } from './renders';
import { defaultState, mockStoreCreator } from './testStores';

let _store;

export const config = ({ component, preloadedState, needsRoutes, startingPath }) => {
  return {
    component,
...
export const displayNot = (arrayOfTestIds, config) => {
  it(`Does not display ${arrayOfTestIds.toString().replaceAll(',', ', ')}`, () => {
    for (const testId of arrayOfTestIds) {
      const { utils } = setupWithMock(config);
...
export const elements = (arrayOfElementIds, config) => {
  return {
    ...
    shouldHaveCorrectRoleWithin: parentId => {
      it(`Elements with data-testids of ${arrayOfElementIds
        .toString()
        .replaceAll(',', ', ')} should have correct role within ${parentId}`, () => {
        const { utils } = setupWithMock(config);
        for (let index = 0; index < arrayOfElementIds.length; index++) {
          try {
            const element = arrayOfElementIds[index];
            const component = utils.getAllByTestId(parentId);
            const role = within(component[0]).getAllByRole(element.split('-')[1]);
            ...
            throw new Error(
              ...
              } and data-testid of ${arrayOfElementIds[index]} within ${parentId}`
            );
          }
        }
      });
    ...
    shouldHaveAttribute: function (attribute, value) {
      it(`Elements with ids of ${arrayOfElementIds
        .toString()
        .replaceAll(',', ', ')} should have property "${attribute}" set to "${value}"`, () => {
        const { utils } = setupWithMock(config);
  ...
  return {
    ...
    changesLocationTo: function (location) {
      return it(`Clicking a Link with data-testid ${id} changes location to ${location}`, async () => {
        const { history, utils } = setupWithMock(config, renderWithReduxAndRouter);
        userEvent.click(await utils.findByTestId(id));
    ...
    addsServerValue: {
      to: function (reducerName) {
        it(`Clicking ${id} results in saving http-response data to ${reducerName}-state`, async () => {
          const component = setupWithMock(config);
          await act(async () => {
            userEvent.click(component.getByTestId(id));
          });

          // Using a bit of a gimmick to ensure the _store is given time to update if there were async functions
          await new Promise((resolve, reject) => {
            setTimeout(resolve, 50);
          });
          ...
          if (Array.isArray(state)) {
            expect(state).toEqual(expect.arrayContaining([serverValue]));
          }
          // else if(typeof currentState === "object"){
          //     // Check if the two objects are equal
          //     if(util.isDeepStrictEqual(currentState, payload)) found = currentState;
          // }
          // else {
          //     // It's a string or a number
          //     if(currentState === state) found = currentState
          // }
        });
      }
    },
    displays: function (arrayOfTestIds) {
      for (const testId of arrayOfTestIds) {
        it(`Clicking ${id} ${amount} time${amount > 1 ? 's' : ''} displays ${testId}`, async () => {
          const { utils } = setupWithMock(config);
          for (let index = 0; index < amount; index++) {
            userEvent.click(utils.getByTestId(id));
          }
    ...
    displaysNot: function (arrayOfTestIds) {
      for (const testId of arrayOfTestIds) {
        it(`Clicking ${id} ${amount} time${amount > 1 ? 's' : ''} hides ${testId}`, async () => {
          const { utils } = setupWithMock(config);
          for (let index = 0; index < amount; index++) {
            userEvent.click(utils.getByTestId(id));
          }
...
export const dispatch = ({ action, value, usesThunk }) => {
  return {
     ...
     */
    when: ({ caseNum, situation, config, FilledInputs, userClickTarget, afterTime = 0 }) => {
      it(`${caseNum ? `Case ${caseNum}: ` : ''}${situation || ''}${
        ...
        if (FilledInputs?.constructor === Object && Object.keys(FilledInputs).length !== 0) {
        ...
        if (usesThunk) {
          expect(store.dispatch.mock.calls[0][0].toString()).toBe(action(value).toString());
        } else {
